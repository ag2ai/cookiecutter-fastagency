...
{% endif %}
# Run uvicorn server
uvicorn {{cookiecutter.project_slug}}.main_{% if "nats" in cookiecutter.app_type %}2_fastapi{% elif "fastapi" in cookiecutter.app_type %}_1_fastapi{% endif %}:app --host 0.0.0.0 --port $FASTAPI_PORT > /dev/stdout 2>&1 &
{% if "mesop" in cookiecutter.app_type %}
# Run gunicorn server
